...
import ItemNotAvailble from '../components/assets/ItemNotAvailable.jpg';
...
const RowContainer = ({flag, data, scrollValue}) => {
  ...
  const addtocart = () => {
    ...
    dispatch({
      type : actionType.SET_CARTITEMS, cartItems : items, 
    });
  ...
  return (
    ...
    {(data && data.length > 0 ? (data.map((item) => (
      <div key={item?.id} className='w-275 h-[225px] min-w-[275px] md:w-300 md:min-w-[300px] bg-orange-100 rounded-lg py-2 px-4 my-12 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-evenly relative'>
    ...
    <div className='w-full flex flex-col items-center justify-center'>
      <img src={ItemNotAvailble} className='h-340' />
...
//6:16:55 if one of the category has no data
//7:01:15